...
\begin{itemize}
	...
	\item (5.8.3)
	      ...
	      Trivialmente, \(S\) é um conjunto independente, uma vez que \(G\) é livre de triângulos. Quando adicionamos \(\mathcal{N}\) à \(S\), sabemos que nenhum desses vértices são vizinhos
	      pois se não existiria um triângulo em \(G\). Seja \(t = |S|\) o tamanho de \(S\), Denotamos por \(\mathbb{E}[t]\) a esperança de \(t\).

	      Sabemos que \(\mathbb{E}[t] = \sum_{v \in V(G)} \mathbb{P}[v \in S]\). Para um \(v\) pertencer a \(S\), nenhum de seus vizinhos pode pertencer, e \(v\) possui \(d(v)\) vizinhos.
	      Para cada vértice \(v\), de acordo com a construção do conjunto \(S\), adicionamos \(d(v)\) a \(S\) e removemos \(v\) e \(d_2(v)\) das possibilidades de escolha.
	      Logo, para cada \(v\), a quantidade esperada de vértices adicionados por ele a \(S\) é de \(\frac{d(v)}{1 + d(v) + d_2(v)}\).
